...
export class FalabellaMapper {
  static toProductDTOs(product: Product): FalabellaProductDTO[] {
    ...
    if (!datasheet) {
      throw new Error(
        `No datasheet found for product ${product.sku} (realm: ${realm} or default)`,
      )
    }
    ...
    const taxClass =
      baseAttributes['tax_percentage'] ||
      baseAttributes['tax_class'] ||
      'Standard'
    const description = datasheet.description
    const name = datasheet.name
    const price = Number(
      baseAttributes['price_falabella'] || baseAttributes['price'] || 0,
    )
    ...
    const sortedImages = [...product.gallery].sort(
    ...
    if (!product.variants || product.variants.length === 0) {
      ...
      return product.variants.map((variant) => {
        return {
          SellerSku:
            variant.sku || `${product.sku}-${variant.variationIds.join('-')}`,
