...
export class WooCommerceApiService {
  ...
  private getOrdersPage(
    ...
    if (!this.api) {
      return throwError(() => new Error('WooCommerce API not initialized'))
    }
    ...
    return from(
      ...
      map((response: { data: any; headers: Record<string, string> }) => {
  ...
  private streamPages(
    page: number,
    perPage: number,
  ): Observable<{ data: WooCommerceOrder[]; page: number; hasMore: boolean }> {
    return this.getOrdersPage(page, perPage).pipe(
  ...
  public getOrdersStream(
    ...
    return this.streamPages(1, perPage).pipe(
      expand((result) =>
        result.hasMore ? this.streamPages(result.page + 1, perPage) : of(),
      ),
      map((result) => result.data),
