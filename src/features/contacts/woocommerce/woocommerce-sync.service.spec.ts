...
describe('WooCommerceSyncService', () => {
  ...
  describe('syncCustomers', () => {
    it('should process orders and create new contacts', (done) => {
      ...
      contactService.findAllPaginated.mockReturnValue(
        of({ data: [], total: 0, page: 1, limit: 1 }),
      )
    ...
    it('should update existing contacts if changed', (done) => {
      ...
      wooCommerceApi.getOrdersStream.mockReturnValue(of(orders))
      contactService.findAllPaginated.mockReturnValue(
        of({ data: [existingContact as any], total: 1, page: 1, limit: 1 }),
      )
      contactService.updateContact.mockReturnValue(
        of({ ...existingContact, firstName: 'John' } as any),
      )
  ...
  describe('contact update', () => {
    ...
    it('should not update contact when data is unchanged', (done) => {
      wooCommerceApi.getOrdersStream.mockReturnValue(of([mockOrder]))
      contactService.findAllPaginated.mockReturnValue(
        of({ data: [mockContact as any], total: 1, page: 1, limit: 1 }),
      )
    ...
    it('should return accurate statistics for mixed operations', (done) => {
      const orders = [
        mockOrder, // Create
        {
          ...mockOrder,
          id: 2,
          billing: { ...mockOrder.billing, email: 'jane@example.com' },
          meta_data: [{ id: 2, key: '_billing_document', value: '987654321' }],
        }, // Create
  ...
  describe('duplicate key error handling', () => {
    it('should handle E11000 duplicate key error and retry as update', (done) => {
      const duplicateError = {
        code: 11000,
        message:
          'E11000 duplicate key error collection: mannaiah.contacts index: email_1 dup key: { email: "john@example.com" }',
      }

      wooCommerceApi.getOrdersStream.mockReturnValue(of([mockOrder]))
      // First lookup returns empty (no contact found)
      contactService.findAllPaginated.mockReturnValueOnce(
        of({ data: [], total: 0, page: 1, limit: 1 }),
      )
  ...
  describe('email deduplication', () => {
    ...
    it('should process orders with different emails separately', (done) => {
      const order1 = {
        ...mockOrder,
        id: 1,
        billing: { ...mockOrder.billing, email: 'john@example.com' },
      }
      const order2 = {
        ...mockOrder,
        id: 2,
    ...
    it('should skip orders without email in deduplication', (done) => {
      ...
      const order3 = {
        ...
        id: 3,
        billing: { ...mockOrder.billing, email: 'jane@example.com' },
        meta_data: [{ id: 3, key: '_billing_document', value: '987654321' }],
      }
